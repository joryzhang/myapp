...
export async function healthCheck(): Promise<RAGResponse> {
  return fetch(`/health`, { method: 'GET' }).then((res) => res.json());
}
...
export async function uploadPDF(file: File): Promise<RAGResponse> {
  ...
  // 使用 fetch 直连 Python，避免 umi request 的额外处理干扰 去掉了${RAG_API_HOST}
...
export async function chatStream(
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({ query, history, temperature }),
  });
