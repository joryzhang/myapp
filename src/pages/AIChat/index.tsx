...
import { Card, Input, Button, Space, Typography, Avatar, message, Collapse, Spin } from 'antd';
import { SendOutlined, RobotOutlined, UserOutlined, ThunderboltOutlined, LoadingOutlined, CaretRightOutlined } from '@ant-design/icons';
...
const useStyles = createStyles(({ token }) => {
    return {
        ...
        chatArea: {
            flex: 1,
            overflowY: 'auto',
            padding: '24px 0 100px 0', // Bottom padding for input area
...
const AIChat: React.FC = () => {
    ...
    const handleSend = async () => {
        ...
        try {
            const response = await chatStream(currentQuestion);
                    ...
                    <Panel
                        header={
                            <Space>
                                ...
                                {isStreaming && isThinking && <Spin indicator={<LoadingOutlined style={{ fontSize: 14 }} spin />} />}
                            </Space>
                        }
                        key="1"
        ...
        <div className={styles.mainContainer}>
            ...
            <div className={styles.chatArea}>
                ...
                {messages.length === 0 && !loading && (
                    <div style={{ textAlign: 'center', marginTop: '100px', opacity: 0.5 }}>
                        <RobotOutlined style={{ fontSize: '64px', color: '#cbd5e1', marginBottom: '16px' }} />
                        <Title level={3} style={{ color: '#94a3b8' }}>How can I help you today?</Title>
                    </div>
                )}

                {/* History Messages */}
                {messages.map((msg) => (
                    <div key={msg.id} className={`${styles.messageRow} ${msg.role === 'user' ? styles.userMessageRow : styles.aiMessageRow}`}>
                        {/* Avatar for AI */}
                        {msg.role === 'assistant' && (
                            <Avatar
                                icon={<RobotOutlined />}
                        ...
                        <div style={{ maxWidth: '100%' }}>
                            ...
                            {streamingContent && (
                                <div className={`${styles.messageBubble} ${styles.aiBubble}`}>
                                    ...
                                    <span style={{ display: 'inline-block', width: '8px', height: '15px', backgroundColor: '#2563EB', marginLeft: '4px', animation: 'blink 1s infinite' }} />
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <div className={styles.inputContainer}>
                <div className={styles.inputWrapper}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '8px' }}>
                        <Button
                            type="primary"
                            shape="circle"
                            icon={loading ? <Spin indicator={<LoadingOutlined style={{ fontSize: 18, color: '#fff' }} spin />} /> : <SendOutlined />}
                            onClick={handleSend}
                ...
                <div style={{ textAlign: 'center', marginTop: '8px' }}>
                    <Text type="secondary" style={{ fontSize: '12px' }}>AI can make mistakes. Please verify important information.</Text>
                </div>
            </div>
        </div>
    );
