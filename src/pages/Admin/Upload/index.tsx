import { healthCheck, uploadPDF } from '@/services/rag';
import {
  CheckCircleFilled,
  CloudUploadOutlined,
  FilePdfOutlined,
    ...
    >
      <div className={styles.container}>
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          <Card
            bordered={false}
            style={{ borderRadius: '16px', background: 'transparent', padding: 0 }}
            bodyStyle={{ padding: 0 }}
          >
            <Alert
              message={
            ...
            <Dragger {...uploadProps} disabled={uploading}>
              <div style={{ padding: '24px 0' }}>
                ...
                <Text type="secondary" style={{ fontSize: '14px' }}>
                  {uploading
                    ? '正在进行智能文本提取和向量存储，请勿关闭页面'
                    : 'Designed for high-accuracy parsing'}
                </Text>
              </div>
            </Dragger>
          </Card>
            ...
            <Card title="本次会话上传记录" bordered={false} style={{ borderRadius: '16px' }}>
              {uploadedFiles.map((filename, index) => (
                <div key={index} className={styles.fileItem}>
                  <FilePdfOutlined
                    style={{ fontSize: '24px', color: '#ff4d4f', marginRight: '16px' }}
                  />
                  <div style={{ flex: 1 }}>
